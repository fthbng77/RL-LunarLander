...
class DQN:
    ...
    # ajanın geçmiş deneyimlerden ders almasını sağlayan fonkdur. 
    ...
    #deneyim tekrar hafızasından rastgele bir örnek alır ve 5 bileşene ayırır.
    def get_attribues_from_sample(self, random_sample):
        ...
        states = np.squeeze(states)# dizi boyutunu küçültür.squeeze tek boyutları kaldırır.
        next_states = np.squeeze(next_states)
        return np.squeeze(states), actions, rewards, next_states, done_list

    # random hareketin tekrarlanmasını sağlar.
    ...
    def train(self, num_episodes=2000, can_stop=True):
        for episode in range(num_episodes):
            ...
            num_steps = 1000
            #print(f'self.num_observation_space: {self.num_observation_space}')
            #print(f'state: {state}')
...
if __name__ == '__main__':
    ...
    rewards_list = pickle.load(open("saved_models" + "train_rewards_list.p", "rb"))
    ...
    pickle.dump(test_rewards, open("saved_models" + "test_rewards.p", "wb"))
    test_rewards = pickle.load(open("saved_models" + "test_rewards.p", "rb"))
